...
impl<'tcx> LateLintPass<'tcx> for IndexRefutableSlice {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx hir::Expr<'_>) {
        if (!expr.span.from_expansion() || is_expn_of(expr.span, "if_chain").is_some())
            ...
            && meets_msrv(self.msrv, msrvs::SLICE_PATTERNS)
    ...
    fn visit_expr(&mut self, expr: &'tcx hir::Expr<'tcx>) {
        if let Some(local_id) = path_to_local(expr) {
            ...
            if let Some(use_info) = slice_lint_info.get_mut(&local_id)
                ...
                && let hir::ExprKind::Index(_, index_expr) = parent_expr.kind
                && let Some((Constant::Int(index_value), _)) = constant(cx, cx.typeck_results(), index_expr)
