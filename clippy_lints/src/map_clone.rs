...
impl<'tcx> LateLintPass<'tcx> for MapClone {
    fn check_expr(&mut self, cx: &LateContext<'_>, e: &hir::Expr<'_>) {
        ...
        if let hir::ExprKind::MethodCall(method, args, _) = e.kind
            && args.len() == 2
            && method.ident.name == sym::map
            && let ty = cx.typeck_results().expr_ty(&args[0])
            && (is_type_diagnostic_item(cx, ty, sym::Option) || is_trait_method(cx, e, sym::Iterator))
            && let hir::ExprKind::Closure(_, _, body_id, _, _) = args[1].kind
        {
            ...
            match closure_body.params[0].pat.kind {
                ...
                hir::PatKind::Binding(hir::BindingAnnotation::Unannotated, .., name, None) => {
                    match closure_expr.kind {
                        ...
                        hir::ExprKind::MethodCall(method, [obj], _) => if ident_eq(name, obj)
