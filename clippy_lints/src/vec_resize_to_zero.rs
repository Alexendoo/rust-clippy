...
impl<'tcx> LateLintPass<'tcx> for VecResizeToZero {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        if let hir::ExprKind::MethodCall(path_segment, args, _) = expr.kind
            ...
            && match_def_path(cx, method_def_id, &paths::VEC_RESIZE) && args.len() == 3
            && let ExprKind::Lit(Spanned { node: LitKind::Int(0, _), .. }) = args[1].kind
            && let ExprKind::Lit(Spanned { node: LitKind::Int(..), .. }) = args[2].kind
        {
