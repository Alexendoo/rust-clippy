...
fn lint_manual_unwrap_or<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
    fn applicable_or_arm<'a>(cx: &LateContext<'_>, arms: &'a [Arm<'a>]) -> Option<&'a Arm<'a>> {
        if arms.len() == 2
            ...
            && (is_lang_ctor(cx, qpath, OptionSome) || is_lang_ctor(cx, qpath, ResultOk))
    ...
    if let ExprKind::Match(scrutinee, match_arms, _) = expr.kind
        ...
        && let Some(ty_name) = if is_type_diagnostic_item(cx, ty, sym::Option) {
            Some("Option")
        ...
        && let Some(or_arm) = applicable_or_arm(cx, match_arms)
