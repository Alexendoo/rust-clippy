...
impl<'tcx> LateLintPass<'tcx> for TryErr {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        ...
        if !in_external_macro(cx.tcx.sess, expr.span)
            && let ExprKind::Match(match_arg, _, MatchSource::TryDesugar) = expr.kind
            && let ExprKind::Call(match_fun, try_args) = match_arg.kind
            && let ExprKind::Path(ref match_fun_path) = match_fun.kind
            && matches!(match_fun_path, QPath::LangItem(LangItem::TryTraitBranch, ..))
            && let Some(try_arg) = try_args.get(0)
            && let ExprKind::Call(err_fun, err_args) = try_arg.kind
            && let Some(err_arg) = err_args.get(0)
            && let ExprKind::Path(ref err_fun_path) = err_fun.kind
            && is_lang_ctor(cx, err_fun_path, ResultErr)
...
fn poll_result_error_type<'tcx>(cx: &LateContext<'tcx>, ty: Ty<'tcx>) -> Option<Ty<'tcx>> {
    if let ty::Adt(def, subst) = ty.kind()
        && match_def_path(cx, def.did(), &paths::POLL)
