...
pub(super) fn check<'tcx>(
    ...
    if same_item_push_visitor.should_lint()
        && let Some((vec, pushed_item)) = same_item_push_visitor.vec_push
                ...
                match cx.qpath_res(qpath, pushed_item.hir_id) {
                    ...
                    Res::Local(hir_id) => {
                        ...
                        if let Node::Binding(pat) = node
                            ...
                            && !matches!(bind_ann, BindingAnnotation::RefMut | BindingAnnotation::Mutable)
...
fn get_vec_push<'tcx>(cx: &LateContext<'tcx>, stmt: &'tcx Stmt<'_>) -> Option<(&'tcx Expr<'tcx>, &'tcx Expr<'tcx>)> {
    if let StmtKind::Semi(semi_stmt) = &stmt.kind
            ...
            && let ExprKind::MethodCall(path, args, _) = &semi_stmt.kind
            // Figure out the parameters for the method call
            && let Some(self_expr) = args.get(0)
            && let Some(pushed_item) = args.get(1)
