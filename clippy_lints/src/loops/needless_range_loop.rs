...
fn is_len_call(expr: &Expr<'_>, var: Symbol) -> bool {
    if let ExprKind::MethodCall(method, len_args, _) = expr.kind
        ...
        && let ExprKind::Path(QPath::Resolved(_, path)) = len_args[0].kind
        && path.segments.len() == 1
...
fn is_end_eq_array_len<'tcx>(
    ...
    if let ExprKind::Lit(ref lit) = end.kind
        ...
        && let Some(arr_len) = arr_len_const.try_eval_usize(cx.tcx, cx.param_env)
    {
        return match limits {
...
impl<'a, 'tcx> Visitor<'tcx> for VarVisitor<'a, 'tcx> {
    fn visit_expr(&mut self, expr: &'tcx Expr<'_>) {
        ...
        if let ExprKind::MethodCall(meth, [args_0, args_1, ..], _) = &expr.kind
            && ((meth.ident.name == sym::index && match_trait_method(self.cx, expr, &paths::INDEX))
