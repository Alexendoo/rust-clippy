...
pub(super) fn check(
    ...
    if let ExprKind::Call(repeat_fn, [repeat_arg]) = take_self_arg.kind
        && is_expr_path_def_path(cx, repeat_fn, &paths::ITER_REPEAT)
        && is_type_diagnostic_item(cx, cx.typeck_results().expr_ty(collect_expr), sym::String)
        && let Some(collect_id) = cx.typeck_results().type_dependent_def_id(collect_expr.hir_id)
        ...
        && cx.tcx.trait_of_item(collect_id) == Some(iter_trait_id)
