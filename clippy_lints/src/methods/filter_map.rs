        ...
        hir::ExprKind::Closure(_, _, c, _, _) => {
            ...
            match closure_expr.kind {
                hir::ExprKind::MethodCall(hir::PathSegment { ident, .. }, args, _) => {
                    ...
                    && let hir::ExprKind::Path(path) = &args[0].kind
                    && let Res::Local(local) = cx.qpath_res(path, args[0].hir_id)
                {
                    return arg_id == local
                }
                    false
...
pub(super) fn check<'tcx>(
    ...
    if is_trait_method(cx, map_recv, sym::Iterator)
            ...
            // filter(|x| ...is_some())...
            && let ExprKind::Closure(_, _, filter_body_id, ..) = filter_arg.kind
            ...
            && let ExprKind::MethodCall(path, [filter_arg], _) = filter_body.value.kind
            && let Some(opt_ty) = cx.typeck_results().expr_ty(filter_arg).ty_adt_def()
            && let Some(is_result) = if cx.tcx.is_diagnostic_item(sym::Option, opt_ty.did()) {
                Some(false)
            ...
            && path.ident.name.as_str() == if is_result { "is_ok" } else { "is_some" }

            // ...map(|x| ...unwrap())
            && let ExprKind::Closure(_, _, map_body_id, ..) = map_arg.kind
            && let map_body = cx.tcx.hir().body(map_body_id)
            && let [map_param] = map_body.params
            && let PatKind::Binding(_, map_param_id, map_param_ident, None) = map_param.pat.kind
            // closure ends with expect() or unwrap()
            && let ExprKind::MethodCall(seg, [map_arg, ..], _) = map_body.value.kind
            ...
            && SpanlessEq::new(cx).expr_fallback(eq_fallback).eq_expr(filter_arg, map_arg)
        {
