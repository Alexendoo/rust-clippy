...
fn detect_hypot(cx: &LateContext<'_>, args: &[Expr<'_>]) -> Option<String> {
    ...
    ) = args[0].kind
    {
        ...
        if let ExprKind::MethodCall(
            ...
            && let Some((lvalue, _)) = constant(cx, cx.typeck_results(), largs_1)
            && let Some((rvalue, _)) = constant(cx, cx.typeck_results(), rargs_1)
...
fn check_expm1(cx: &LateContext<'_>, expr: &Expr<'_>) {
    if let ExprKind::Binary(Spanned { node: BinOpKind::Sub, .. }, lhs, rhs) = expr.kind
        ...
        && let Some((value, _)) = constant(cx, cx.typeck_results(), rhs)
...
fn are_same_base_logs(cx: &LateContext<'_>, expr_a: &Expr<'_>, expr_b: &Expr<'_>) -> bool {
    if let ExprKind::MethodCall(PathSegment { ident: method_name_a, .. }, args_a, _) = expr_a.kind
        && let ExprKind::MethodCall(PathSegment { ident: method_name_b, .. }, args_b, _) = expr_b.kind
    {
...
fn check_log_division(cx: &LateContext<'_>, expr: &Expr<'_>) {
    ...
    if let ExprKind::Binary(
        ...
        && let ExprKind::MethodCall(_, [largs_self, ..], _) = &lhs.kind
        && let ExprKind::MethodCall(_, [rargs_self, ..], _) = &rhs.kind
    {
        span_lint_and_sugg(
            cx,
            SUBOPTIMAL_FLOPS,
...
fn check_radians(cx: &LateContext<'_>, expr: &Expr<'_>) {
    if let ExprKind::Binary(
        ...
        && let Some((rvalue, _)) = constant(cx, cx.typeck_results(), div_rhs)
        && let Some((lvalue, _)) = constant(cx, cx.typeck_results(), mul_rhs)
    {
