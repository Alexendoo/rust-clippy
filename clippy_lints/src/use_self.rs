...
impl<'tcx> LateLintPass<'tcx> for UseSelf {
    fn check_item(&mut self, _cx: &LateContext<'_>, item: &Item<'_>) {
        ...
        let stack_item = if let ItemKind::Impl(Impl { self_ty, .. }) = item.kind
    ...
    fn check_ty(&mut self, cx: &LateContext<'_>, hir_ty: &hir::Ty<'_>) {
        if !hir_ty.span.from_expansion()
            && meets_msrv(self.msrv, msrvs::TYPE_ALIAS_ENUM_VARIANTS)
            ...
            && same_type_and_consts(ty, cx.tcx.type_of(impl_id))
            && let hir = cx.tcx.hir()
            // prevents false positive on `#[derive(serde::Deserialize)]`
            && !hir.span(hir.get_parent_node(hir_ty.hir_id)).in_derive_expansion()
        {
    ...
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        if !expr.span.from_expansion()
            ...
            && cx.typeck_results().expr_ty(expr) == cx.tcx.type_of(impl_id)
        {} else { return; }
        match expr.kind {
    ...
    fn check_pat(&mut self, cx: &LateContext<'_>, pat: &Pat<'_>) {
        if !pat.span.from_expansion()
            && meets_msrv(self.msrv, msrvs::TYPE_ALIAS_ENUM_VARIANTS)
            && let Some(&StackItem::Check { impl_id, .. }) = self.stack.last()
            && let PatKind::Path(QPath::Resolved(_, path)) = pat.kind
            && !matches!(path.res, Res::SelfTy { .. } | Res::Def(DefKind::TyParam, _))
            && cx.typeck_results().pat_ty(pat) == cx.tcx.type_of(impl_id)
            && let [first, ..] = path.segments
            && let Some(hir_id) = first.hir_id
        {
