...
impl<'tcx> LateLintPass<'tcx> for GetLastWithLen {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        ...
        if let ExprKind::MethodCall(path, args, _) = expr.kind
            ...
            // Method name is "get"
            && path.ident.name == sym!(get)

            // Argument 0 (the struct we're calling the method on) is a vector
            && let Some(struct_calling_on) = args.get(0)
            && let struct_ty = cx.typeck_results().expr_ty(struct_calling_on)
            && is_type_diagnostic_item(cx, struct_ty, sym::Vec)
            ...
            && let ExprKind::MethodCall(arg_lhs_path, lhs_args, _) = &lhs.kind
            && arg_lhs_path.ident.name == sym::len
            && let Some(arg_lhs_struct) = lhs_args.get(0)

            // The two vectors referenced (x in x.get(...) and in x.len())
            && SpanlessEq::new(cx).eq_expr(struct_calling_on, arg_lhs_struct)
