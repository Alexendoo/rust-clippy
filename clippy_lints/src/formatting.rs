    ...
    if let ExprKind::Array(ref array) = expr.kind {
        for element in array {
            if let ExprKind::Binary(ref op, ref lhs, _) = element.kind
                && has_unary_equivalent(op.node) && lhs.span.ctxt() == op.span.ctxt()
...
fn check_missing_else(cx: &EarlyContext<'_>, first: &Expr, second: &Expr) {
    if !first.span.from_expansion() && !second.span.from_expansion()
        && let ExprKind::If(cond_expr, ..) = &first.kind
        && (is_block(second) || is_if(second))

        // Proc-macros can give weird spans. Make sure this is actually an `if`.
        && let Some(if_snip) = snippet_opt(cx, first.span.until(cond_expr.span))
        && if_snip.starts_with("if")
